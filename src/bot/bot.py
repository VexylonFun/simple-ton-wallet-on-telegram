...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(
        [[
            InlineKeyboardButton(
                f"Claim {amount} TON",
                url=f"https://t.me/SimpleTonWallet_bot?start=accept-{transfer_uuid}",
            )
        ]]
    )
    message = InputTextMessageContent(message_text=f"Проверьте сумму <b>{amount}</b> TON", parse_mode=ParseMode.HTML)
