...
async def deeplink_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not func:
        ...
        context.user_data["msg"].edit_text("⚠️ Неверная ссылка", reply_markup=reply_markup)
        return
...
async def accept_transfer_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if transfer_data is None:
        await update.message.reply_text("⚠️ перевод был погашен или отменен")
        return
    ...
    if wallet.balance < amount:
        await update.message.reply_text("⚠️ На балансе отправителя недостаточно средств")
        return
    ...
    try:
        ...
        await msg.edit_text("⚠️ Неизвестная ошибка при переводе средств")
        return
    else:
        if not self_wallet.initialized:
            await self_wallet.initialize()

    await msg.edit_text(f"✅ Вы получили: {amount} TON")
...
async def new_password_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.user_data["msg"].edit_text("Пожалуйста, пришлите пароль еще раз:")
...
async def check_password_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_password_valid(password, password_data["hash"], password_data["salt"]):
        ...
        await context.user_data["msg"].edit_text("Неверный пароль!.\nПожалуйста, введите свой пароль:")
...
async def confirm_password_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_password_valid(password, password_data["hash"], password_data["salt"]):
        ...
        await context.user_data["msg"].edit_text("Пароли не совпали.\nПожалуйста, выберите пароль:")
        return "new-password"
...
async def send_address_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(f"Мин: {min_send}", callback_data="min_send"), InlineKeyboardButton(f"Макс: {balance-fee}", callback_data="max_send")],
        [InlineKeyboardButton("Отмена", callback_data="cancel-send")],
    ]
...
async def send_amount_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, amount: float = None):
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅Да", callback_data="send-confirm"),
            InlineKeyboardButton("❌Нет", callback_data="cancel-send")
        ],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def send_confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if datetime.datetime.now() - context.user_data["last_login"] < datetime.timedelta(minutes=1):
        ...
        if is_unbounceable_address(send_address) and send_amount > 2:
            await context.bot.send_message(chat_id, "Вы не можете отправить более 2 TON на необъединяемый адрес.")
            return
...
async def settings_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(text="Показать 24 слова", callback_data="wordlist")],
